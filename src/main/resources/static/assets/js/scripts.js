...
*/
        ...
        var API = function () {
                ...
                function connectToIndexingStatus() {
                    const eventSource = new EventSource('/api/indexingStatus');

                    // Обработчик для события 'indexingStatus'
                    eventSource.addEventListener('indexingStatus', function (event) {
                        const message = event.data;
                        console.log('Received indexing status:', message);

                        // Проверяем сообщение об успешной индексации
                        if (message === 'Indexing completed') {
                            // Изменяем текст кнопки и другие атрибуты только если индексация завершена успешно
                                            ...
                                            action: function (result, $this) {
                                                if (result.result) {
                                                    ...
                                                    result.statistics.detailed.forEach(function (site) {
                                                        ...
                                                        // Получение элемента <select> по имени
                                                        let selectElement = document.querySelector('select[name="site"]');
                                                        let newValue = site.url;
